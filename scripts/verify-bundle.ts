...
if (
  ...
  !("captures" in provenanceValue) ||
  !Array.isArray(provenanceValue.captures) ||
  provenanceValue.captures.length < 2 ||
  provenanceValue.captures.length > 4
) {
...
for (const [filename, expected] of [
  ...
  const { stdout } = await execFileAsync(
    ...
    [
      ...
      "error",
      "-show_entries",
      "stream=codec_type,codec_name,profile,pix_fmt,width,height,r_frame_rate,duration,bit_rate",
      "-of",
      "json",
      path,
    ],
  ...
  if (
    ...
    duration < 25 ||
    duration > 40 ||
    (production && (!Number.isFinite(bitrate) || bitrate < 2_000_000))
  ) {
